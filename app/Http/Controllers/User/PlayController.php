...
class PlayController extends Controller {
    ...
    public function playGame($alias) {
        $game      = Service::where('alias', $alias)->firstOrFail();
        // return $game;
        $pageTitle = "Puchase " . $game->name;
        // return $game->type;
        // if($game->type =="construct"){
        //     return view($this->activeTemplate . 'user.games.'. $alias.'.index', compact('game', 'pageTitle'));
        // }

        // $defaults = ['head_tail', 'rock_paper_scissors', 'spin_wheel', 'number_guess', 'dice_rolling',
        // 'card_finding', 'number_slot', 'number_pool'];

        // if(in_array($alias, $defaults)){
            return view($this->activeTemplate . 'user.services.' . $alias, compact('game', 'pageTitle'));
        // }
        // return view($this->activeTemplate . 'user.games.construct', compact('game', 'pageTitle'));
    }
